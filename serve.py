...
class SimpleLitAPI(LitAPI):
    ...
    def decode_request(self, request: Dict[str, Any]) -> Any:
        ...
        # !!!CUSTOMIZED SEGMENT!!!
        #   // Added 'input' to 'apply()' so that the model also accounts for passed context
        #   Vanilla code:
        #   prompt = request["prompt"]
        #   prompt = self.prompt_style.apply(prompt)
        #   Modified code:
        prompt = self.prompt_style.apply(prompt=request["prompt"], input=request["input"])  #} 
        # !!!END OF CUSTOMIZED SEGMENT!!!
